...
<div class="content-wrapper">
    <div class="page-header">
        <h3 class="page-title"> Edit Toko </h3>
    </div>
        ...
        <div class="col-md-8 grid-margin stretch-card">
            <div class="card" style="background: rgba(255, 255, 255, 0.9); backdrop-filter: blur(10px); border-radius: 15px;">
                <div class="card-body">
                    ...
                    <form class="forms-sample" action="{{ route('toko.update', $toko->idtoko) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="form-group">
                            <label for="barcode">Barcode</label>
                            <input type="text" class="form-control" id="barcode" name="barcode" value="{{ old('barcode', $toko->barcode) }}" required>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="row">
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="latitude" name="latitude" value="{{ old('latitude', $toko->latitude) }}" readonly required>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label>Longitude</label>
                                    <input type="text" class="form-control" id="longitude" name="longitude" value="{{ old('longitude', $toko->longitude) }}" readonly required>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label>Akurasi (meter)</label>
                                    <input type="text" class="form-control" id="accuracy" name="accuracy" value="{{ old('accuracy', $toko->accuracy) }}" readonly>
                                </div>
                            </div>
                        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    btnAmbil.addEventListener('click', function() {
        if (!navigator.geolocation) {
            statusText.innerHTML = "<span class='text-danger'>Geolocation tidak didukung.</span>";
            return;
        }
        ...
        navigator.geolocation.getCurrentPosition(
            function(position) {
                ...
                statusText.innerHTML = "<span class='text-success'>Lokasi berhasil diperbarui!</span>";
                btnAmbil.disabled = false;
            },
            function(error) {
                statusText.innerHTML = "<span class='text-danger'>Gagal mendapatkan lokasi.</span>";
                btnAmbil.disabled = false;
            },
            {
